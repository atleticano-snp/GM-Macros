...
async function ssbestZone(token) {
    ...
    for (let i of templates) {
        ...
        if (testOriginItem.name != 'Sleet Storm') continue;
...
async function sleetinZone(token, template) {
    let originUuid = template.flags['midi-qol']?.originUuid;
    if (!originUuid) return;
    if (chris.inCombat()) {
        let turnEntered = template.flags['chris-premades']?.spell?.sleetstorm?.tokens?.[token.id];
        let currentTurn = game.combat.round + '-' + game.combat.turn;
        if (currentTurn === turnEntered) return;
        await template.setFlag('chris-premades', 'spell.sleetstorm.tokens.' + token.id, currentTurn);
    }
    let bestOriginUuid = await ssbestZone(token);
    ...
    let effect = chris.findEffect(token.actor, 'Sleet Storm');
    if (effect) {
...
async function sleetleaveZone(token, template) {
    if (chris.inCombat()) {
        await template.setFlag('chris-premades', 'spell.sleetstorm.tokens.' + token.id, '');
    }
    let effect = chris.findEffect(token.actor, 'Sleet Storm');
    if (!effect) return;
    let originUuid = template.flags['midi-qol']?.originUuid;
...
export let sleetstorm = {
    'sleetinZone': sleetinZone,
    'sleetleaveZone': sleetleaveZone
}
