...
async function greasebestZone(token) {
    ...
    for (let i of templates) {
        ...
        if (testOriginItem.name != 'Grease') continue;
...
async function greaseInZone (token, template) {
    let originUuid = template.flags['midi-qol']?.originUuid;
    if (!originUuid) return;
    if (chris.inCombat()) {
        let turnEntered = template.flags['chris-premades']?.spell?.grease?.tokens?.[token.id];
        let currentTurn = game.combat.round + '-' + game.combat.turn;
        if (currentTurn === turnEntered) return;
        await template.setFlag('chris-premades', 'spell.grease.tokens.' + token.id, currentTurn);
    }
    let bestOriginUuid = await greasebestZone(token);
    ...
    let effect = chris.findEffect(token.actor, 'Grease');
    if (effect) {
...
async function greaseOutZone(token, template) {
    if (chris.inCombat()) {
        await template.setFlag('chris-premades', 'spell.grease.tokens.' + token.id, '');
    }
    let effect = chris.findEffect(token.actor, 'Grease');
    if (!effect) return;
    let originUuid = template.flags['midi-qol']?.originUuid;
...
export let grease = {
    'greaseInZone': greaseInZone,
    'greaseOutZone': greaseOutZone
}
